...
var createChecksumStream = ({ expectedChecksum, checksum, source, checksumSourceLocation, base64Encoder }) => {
  ...
  if (!isReadableStream(source)) {
    throw new Error(`@smithy/util-stream: unsupported source type ${(_b = (_a = source == null ? void 0 : source.constructor) == null ? void 0 : _a.name) != null ? _b : source} in ChecksumStream.`);
  }
...
function sizeOf(chunk) {
  var _a, _b;
  return (_b = (_a = chunk == null ? void 0 : chunk.byteLength) != null ? _a : chunk == null ? void 0 : chunk.length) != null ? _b : 0;
}
...
var recursionDetectionMiddleware = (options) => (next) => async (args) => {
  ...
  const traceIdHeader = (_b = Object.keys((_a = request.headers) != null ? _a : {}).find((h2) => h2.toLowerCase() === TRACE_ID_HEADER_NAME.toLowerCase())) != null ? _b : TRACE_ID_HEADER_NAME;
...
var getRuntimeConfig = (config) => {
  ...
  return {
    ...
    useArnRegion: (_o = config == null ? void 0 : config.useArnRegion) != null ? _o : void 0,
...
var S3AttachmentUploader = class extends import_obsidian.Plugin {
  async onload() {
  ...
  async uploadAllAttachments() {
    if (!this.s3) {
      new import_obsidian.Notice(this.i18n.t("notices.pleaseConfigureS3"));
      return;
    }
    const attachments = this.app.vault.getFiles().filter(
      (file) => this.isAttachment(file)
    );
    if (attachments.length === 0) {
      new import_obsidian.Notice(this.i18n.t("notices.noAttachmentsFound"));
      return;
    }
    new import_obsidian.Notice(this.i18n.t("notices.startingUpload", { count: attachments.length.toString() }));
    for (const attachment of attachments) {
      await this.uploadAndReplaceAttachment(attachment);
    }
    new import_obsidian.Notice(this.i18n.t("notices.allAttachmentsUploaded"));
  }
  async uploadCurrentFileAttachments() {
    ...
    if (!activeFile) {
      new import_obsidian.Notice(this.i18n.t("notices.noActiveFile"));
      return;
    }
    const content = await this.app.vault.read(activeFile);
    const attachmentPaths = this.extractAttachmentPaths(content);
    if (attachmentPaths.length === 0) {
      new import_obsidian.Notice(this.i18n.t("notices.noAttachmentsInCurrentFile"));
      return;
    }
    new import_obsidian.Notice(this.i18n.t("notices.uploadingAttachments", { count: attachmentPaths.length.toString() }));
    let successCount = 0;
    let failCount = 0;
    ...
    if (failCount === 0) {
      new import_obsidian.Notice(this.i18n.t("notices.uploadSuccess", { count: successCount.toString() }));
    } else {
      new import_obsidian.Notice(this.i18n.t("notices.uploadPartialSuccess", {
        success: successCount.toString(),
        failed: failCount.toString()
      }));
    }
  }
  async uploadAndReplaceAttachment(attachment, targetFile) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (this.settings.organizeByDate) {
          const datePath = this.formatDate(/* @__PURE__ */ new Date(), this.settings.dateFormat);
        ...
        if (attempt > 1) {
          new import_obsidian.Notice(this.i18n.t("notices.retryingUpload", {
    ...
    new import_obsidian.Notice(this.i18n.t("notices.uploadFailedAllRetries", {
      filename: attachment.name,
      maxRetries: maxRetries.toString(),
      error: (lastError == null ? void 0 : lastError.message) || "Unknown error"
    }));
    throw lastError;
  }
  async replaceAttachmentReferences(attachment, cloudUrl, targetFile) {
  ...
  isAttachment(file) {
    const allowedExts = this.settings.allowedExtensions.split(",").map((ext) => ext.trim().toLowerCase()).filter((ext) => ext.length > 0);
  ...
  isAttachmentPath(path) {
    ...
    if (!ext) return false;
    const allowedExts = this.settings.allowedExtensions.split(",").map((ext2) => ext2.trim().toLowerCase()).filter((ext2) => ext2.length > 0);
  ...
  async testS3Connection() {
    ...
    try {
      ...
      await this.s3.send(command);
      new import_obsidian.Notice(this.i18n.t("notices.s3ConnectionSuccess"));
      return true;
    } catch (error) {
      console.error("S3 connection test failed:", error);
      new import_obsidian.Notice(this.i18n.t("notices.s3ConnectionFailed", { error: error.message }));
      return false;
    }
  }
  formatDate(date, format) {
    ...
    return format.replace(/YYYY/g, year.toString()).replace(/YY/g, year.toString().slice(-2)).replace(/MM/g, month).replace(/DD/g, day).replace(/HH/g, hour).replace(/mm/g, minute).replace(/ss/g, second);
  }
      ...
      try {
        ...
        if (result.Contents) {
          for (const object of result.Contents) {
            if (object.Key) {
              if (!object.Key.startsWith(this.settings.trashPath + "/") && !object.Key.includes("/" + this.settings.trashPath + "/")) {
  ...
  async findAllObsidianReferences() {
...
var S3UploaderSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.accessKeyId.name")).setDesc(this.plugin.i18n.t("settings.accessKeyId.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.accessKeyId.placeholder")).setValue(this.plugin.settings.accessKeyId).onChange(async (value) => {
      this.plugin.settings.accessKeyId = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.secretAccessKey.name")).setDesc(this.plugin.i18n.t("settings.secretAccessKey.desc")).addText((text) => {
      text.inputEl.type = "password";
      text.setPlaceholder(this.plugin.i18n.t("settings.secretAccessKey.placeholder")).setValue(this.plugin.settings.secretAccessKey).onChange(async (value) => {
        this.plugin.settings.secretAccessKey = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.region.name")).setDesc(this.plugin.i18n.t("settings.region.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.region.placeholder")).setValue(this.plugin.settings.region).onChange(async (value) => {
      this.plugin.settings.region = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.bucketName.name")).setDesc(this.plugin.i18n.t("settings.bucketName.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.bucketName.placeholder")).setValue(this.plugin.settings.bucketName).onChange(async (value) => {
      this.plugin.settings.bucketName = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.baseUrl.name")).setDesc(this.plugin.i18n.t("settings.baseUrl.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.baseUrl.placeholder")).setValue(this.plugin.settings.baseUrl).onChange(async (value) => {
      this.plugin.settings.baseUrl = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.folderPath.name")).setDesc(this.plugin.i18n.t("settings.folderPath.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.folderPath.placeholder")).setValue(this.plugin.settings.folderPath).onChange(async (value) => {
      this.plugin.settings.folderPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.allowedExtensions.name")).setDesc(this.plugin.i18n.t("settings.allowedExtensions.desc")).addTextArea((text) => {
      text.setPlaceholder(this.plugin.i18n.t("settings.allowedExtensions.placeholder")).setValue(this.plugin.settings.allowedExtensions).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.organizeByDate.name")).setDesc(this.plugin.i18n.t("settings.organizeByDate.desc")).addToggle((toggle) => toggle.setValue(this.plugin.settings.organizeByDate).onChange(async (value) => {
      this.plugin.settings.organizeByDate = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (this.plugin.settings.organizeByDate) {
      new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.dateFormat.name")).setDesc(this.plugin.i18n.t("settings.dateFormat.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.dateFormat.placeholder")).setValue(this.plugin.settings.dateFormat).onChange(async (value) => {
        this.plugin.settings.dateFormat = value;
        await this.plugin.saveSettings();
      }));
      const previewSetting = new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.preview.name")).setDesc("");
    ...
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.usePathStyle.name")).setDesc(this.plugin.i18n.t("settings.usePathStyle.desc")).addToggle((toggle) => toggle.setValue(this.plugin.settings.usePathStyle).onChange(async (value) => {
      this.plugin.settings.usePathStyle = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.retryCount.name")).setDesc(this.plugin.i18n.t("settings.retryCount.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.retryCount.placeholder")).setValue(this.plugin.settings.retryCount.toString()).onChange(async (value) => {
      const retryCount = parseInt(value) || 3;
      this.plugin.settings.retryCount = Math.max(1, Math.min(10, retryCount));
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.testConnection.name")).setDesc(this.plugin.i18n.t("settings.testConnection.desc")).addButton((button) => button.setButtonText(this.plugin.i18n.t("settings.testConnection.button")).setCta().onClick(async () => {
    ...
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.trashPath.name")).setDesc(this.plugin.i18n.t("settings.trashPath.desc")).addText((text) => text.setPlaceholder(this.plugin.i18n.t("settings.trashPath.placeholder")).setValue(this.plugin.settings.trashPath).onChange(async (value) => {
      this.plugin.settings.trashPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.moveToTrash.name")).setDesc(this.plugin.i18n.t("settings.moveToTrash.desc")).addToggle((toggle) => toggle.setValue(this.plugin.settings.moveToTrash).onChange(async (value) => {
      this.plugin.settings.moveToTrash = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.i18n.t("settings.cleanupFiles.name")).setDesc(this.plugin.i18n.t("settings.cleanupFiles.desc")).addButton((button) => button.setButtonText(this.plugin.i18n.t("settings.cleanupFiles.button")).setWarning().onClick(async () => {
...
var CleanupModal = class extends import_obsidian.Modal {
  ...
  async startCleanup() {
    ...
    try {
      ...
      if (this.shouldCancel) return;
      const normalizedReferences = /* @__PURE__ */ new Set();
      ...
      let deleted = 0;
      let failed = 0;
      const progressBar = this.progressEl.querySelector("div");
  ...
  showResults(processed, deleted, failed, cancelled) {
    this.resultEl.empty();
    if (cancelled) {
